...
export default async function CountryPage({ params }: Props) {
  ...
  const response = await fetch(
    `${baseUrl}/api/places/${params.country}/${params.province}`,
    {
      next: {
        revalidate: false,
      },
  ...
  return (
    <TitledPage
      title={params.province.capitalize()}
      titlePostfix={params.country.capitalize()}
      separator={SeparatorTypes.none}
      backLink={`/places/${params.country}`}
    >
      <div>
        {data.map((municipality: string) => (
          <div key={municipality}>
            <Link
              href={`/places/${params.country}/${params.province}/${municipality}`}
            >
              {municipality.capitalize()}
            </Link>
          </div>
        ))}
      </div>
